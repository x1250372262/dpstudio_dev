...
import com.dpstudio.dev.doc.core.tag.RespObjTagImpl;
...
public class RespObjTagConverter extends SeeTagConverter {

    @Override
    public DocTag converter(String comment) {
        SeeTagImpl seeTag = (SeeTagImpl) super.converter(comment);
        if (seeTag != null) {
            return new RespObjTagImpl(seeTag.getTagName(), seeTag.getValues());
        }
        return null;
    }
}
